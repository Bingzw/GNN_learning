...
def train_and_validate(train_loader, model, optimizer, num_epochs):
...
if __name__ == "__main__":
    ...
    dataset = Planetoid(root=download_path, name='PubMed', transform=NormalizeFeatures())
    print(dataset[0])
        ...
        for wd in weight_decay:
            for dim in gcn_dim:
                for dp in dropout:
                    ...
                    model, val_acc = train_and_validate(train_loader, model, optimizer, num_epochs)
    ...
    torch.save(best_model.state_dict(), os.path.join(save_dir, 'cluster_gcn_best_model.pt'))
